...
def dataset_factory(val_ratio: int = 0.1):
    metadata = dataset_metadata()
    dataset = datasets.load_dataset("csv", data_files=DATASET_PATH)['train'].train_test_split(test_size=val_ratio)
...
@click.option('--learning_rate', default=1e-5, type=float)
...
def train(
    ...
    # Setup training with LoRA
    def compute_loss(output, labels, return_outputs=False, num_items_in_batch=None, weight=metadata['class_weights']):
